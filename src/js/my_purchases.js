...
class My_purchases extends React.Component {
    ...
    async load_nfts() {
        ...
        const data = await market_contract.fetchMyNFTs();
    ...
    render() {
        return (
            <div>
                ...
                {this.state.nfts.length == 0 ?<h1 id="no_purchases">No Purchases Yet</h1> :null}
                <div>
                    <p>&nbsp;</p>
                </div>
                <div className='grid_container_2_explore'>
                    {
                        this.state.nfts.map((nft, i) => (
                            <div key={i} className="card_2_explore">
                                <button onClick={() => this.togglePopup("my_purchases",i)} id="close">
